...
Fee 榜使用仓库内的真实快照 `uniswap-fee.json`。它由 GitHub Actions 每 15 分钟从 Uniswap ExploreStats 公共接口刷新；页面每 60 秒读取快照，读取失败时仅保留浏览器缓存，不生成 mock 数据。页面分别显示 `RWA / USDC` 与排除 RWA 后的普通币费用前三；当前数据不足三条时按真实数量显示，不补占位数据。
