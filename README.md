...
Uniswap 手续费榜使用仓库内的真实快照 `uniswap-fee.json`。它由 GitHub Actions 每 15 分钟从 Uniswap ExploreStats 公共接口刷新；页面按轮询读取，读取失败时仅保留浏览器缓存，不生成模拟数据。页面显示 `实物资产 / USDC` 与排除实物资产后的普通代币费用前三；当前数据不足三条时按真实数量显示，不补占位数据。Solana LP 费率榜单已从页面和导航移除。
...
token 行情与 Uniswap Fee 使用同一个紧凑容器承载，但不混合价格和费用语义；静态页不加载或展示 HIP-3 股票资金费率榜。
