...
- Hyperliquid：公共 `info` 接口中当前小时资金费率最高的 Top 5
- 新闻：公开的律动 `feed.json`
...
页面每 30 秒直接调用 Hyperliquid 公共 `info` 接口的 `metaAndAssetCtxs`，按当前小时资金费率显示最高 5 个永续市场，并链接到 [Hyperliquid Trade](https://app.hyperliquid.xyz/trade)。该只读接口不需要 API key；请求失败时保留浏览器缓存，不生成 mock 数据。
